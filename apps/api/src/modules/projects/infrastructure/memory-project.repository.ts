...
export class MemoryProjectRepository implements ProjectRepository {
  ...
  list(organizationId: string) { return Promise.resolve(this.records.filter(item => item.organizationId === organizationId && !item.deletedAt)); }
  find(organizationId: string, id: string) { return Promise.resolve(this.records.find(item => item.organizationId === organizationId && item.id === id && !item.deletedAt) ?? null); }
  ...
  update(organizationId: string, id: string, expectedRevision: number, name: string) {
    const record = this.records.find(item => item.organizationId === organizationId && item.id === id && item.revision === expectedRevision && !item.deletedAt);
