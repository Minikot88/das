...
export class ConnectionsController {
  ...
  @Post('test') testCredentials(@Body() body: Record<string, unknown>) { return this.connections.testCredentials(body || {}); }
  @Post(':id/test') test(@CurrentPrincipal() p: RequestPrincipal, @Param('id') id: string) { return this.connections.test(p, id); }
  @Get(':id/schema') schema(@CurrentPrincipal() p: RequestPrincipal, @Param('id') id: string) { return this.connections.discover(p, id); }
  @Post(':id/query') query(@CurrentPrincipal() p: RequestPrincipal, @Param('id') id: string, @Body() body: { sql?: string; parameters?: unknown[] }, @Req() request: FastifyRequest) { return this.connections.execute(p, id, String(body?.sql || ''), Array.isArray(body?.parameters) ? body.parameters : [], ensureRequestId(request)); }
