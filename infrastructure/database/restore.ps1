...
try {
  ...
  docker compose exec -T postgres psql --set=ON_ERROR_STOP=1 --username=$migrationUser --dbname=$temporaryDatabase --set=runtime_user=$runtimeUser --command='REVOKE CREATE ON SCHEMA public FROM PUBLIC; GRANT USAGE ON SCHEMA dashboard_core TO :"runtime_user"; GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA dashboard_core TO :"runtime_user"; GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA dashboard_core TO :"runtime_user"; GRANT USAGE ON SCHEMA scopus TO :"runtime_user"; REVOKE INSERT, UPDATE, DELETE, TRUNCATE, REFERENCES, TRIGGER ON ALL TABLES IN SCHEMA scopus FROM :"runtime_user"; GRANT SELECT ON ALL TABLES IN SCHEMA scopus TO :"runtime_user";'
  if ($LASTEXITCODE -ne 0) { throw 'Could not restore PostgreSQL runtime privileges.' }
  $verificationQuery = 'SELECT (SELECT COUNT(*) FROM public._prisma_migrations), (SELECT COUNT(*) FROM dashboard_core.bi_projects), (SELECT COUNT(*) FROM dashboard_core.datasets), (SELECT COUNT(*) FROM dashboard_core.charts), (SELECT COUNT(*) FROM dashboard_core.bi_dashboards), (SELECT COUNT(*) FROM dashboard_core.dashboard_widgets);'
